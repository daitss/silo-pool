...
module Store
  ...
  class PoolFixity
    ...
    # only option currently supported is :stored_before, which should be a DateTime object
    ...
    def summary
      ...
      conditions[:initial_timestamp.lt] = @options[:stored_before] if @options[:stored_before]
      ...
      Struct::FixityHeader.new(@base_url.host,
                               ...
                               (@options[:stored_before] || DateTime.now).to_utc.to_s
                               )
    end
