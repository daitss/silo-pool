...
# TODO: get the lock file out of the silo - need a /var/run/silos directory, or something....
...
module Store
  ...
  class Silo
    ...
    attr_reader :filesystem    # note: no trailing slash on this when read.
    ...
    # Raises SiloError if path is not a directory.

    ## TODO: it needs to be at least readable...
    ...
    def put(name, data, type = nil)
      ...
      if File.exists? path(name)
        raise SiloResourceExists, "#{self} error - resource #{name} already exists. You must delete this resource first." 
      end

      if (name.length > MAX_NAME_LENGTH or invalid_name? name)
        raise SiloBadName, "#{self} error - invalid name for resource '#{name}'." 
      end
      ...
      write_lock(name) do
        begin
          ...
          raise SiloError, "#{self} directory creation for #{name} failed: #{e.message}"
        end
        ...
        begin
          ...
          raise SiloError, "#{self} can't write the '#{metafile}' metafile for  #{name}: #{e.message}"
        end
        ...
        unless 6 == File.chmod(RESTRICTIVE_FILE_PERMISSIONS, 
          ...
          sha1_path(name), 
          type_path(name))
          raise SiloError, "#{self} can't set restrictive permissions on all data files for #{describe name}" 
        end

        unless 1 == File.chmod(RESTRICTIVE_DIRECTORY_PERMISSIONS, path(name))
          raise SiloError, "#{self} can't set restrictive permissions on data file directory for #{describe name}"
        end
      end # write_lock
    end # def
    ...
    def get(name)
      ...
      read_lock(name) do
        begin
          ...
          raise SiloError, "#{self} can't get #{describe name}: #{e.message}"
        end
      end
    end
    ...
    def delete(name)
      write_lock(name) do
        begin
          ...
          [ datetime_path(name), 
            ...
            raise SiloError, "Can't delete #{describe name}: #{e.message}"
          end
        end
      end
      ...
      def copy_to_silo(name, destination_silo)
        if not self.exists?(name)
          raise SiloError, "Resource #{name} does not exist in silo #{self}."  
        end
        if destination_silo.exists?(name)
           raise SiloResourceExists, "Resource #{name} already exists in silo #{destination_silo.filesystem}."  
        end
        write_lock(name) do
          begin
            open(data_path(name), "r")  do |input|   # use block form of open to ensure close on error
              destination_silo.put(name, input)
            end
          end    
        end # write lock
      end
      ...
      def move_to_silo(name, destination_silo)
        if (StoreUtils.disk_id(self.filesystem) == StoreUtils.disk_id(destination_silo.filesystem))
          move_to_silo_with_rename(name, destination_silo)
        else
          move_to_silo_with_copy(name, destination_silo)
        end
      end

      def move_to_silo_with_copy(name, destination_silo)
        copy_to_silo(name, destination_silo)
        delete(name)
      end

      def move_to_silo_with_rename(name, destination_silo)
        unless exists?(name)
          raise SiloError, "Resource #{name} does not exist in silo #{self.filesystem}."  
        end
        if destination_silo.exists?(name)
          raise SiloResourceExists, "Resource #{name} exists in silo #{destination_silo.filesystem}."  
        end
        destination_silo.write_lock(name) do
          self.write_lock(name) do
            begin
              File.chmod(PERMISSIVE_DIRECTORY_PERMISSIONS,self.path(name))
              FileUtils.mv(self.path(name), destination_silo.parent_path(name))
              File.chmod(RESTRICTIVE_DIRECTORY_PERMISSIONS,destination_silo.path(name))
            rescue => e
              raise SiloError, "Can't correctly move #{describe name} from silo #{self.filesystem} to #{destination_silo.filesystem}: #{e.message}"
            end
          end # of source write_lock
        end # of destination write_lock
      end
      ...
      def size(name)
        read_lock(name) do
          begin
            ...
            raise SiloError, "Can't determine size for #{describe name}: #{e.message}"
          end
        end
      end

      # TODO: implement saving the mime type
      ...
      def type(name)
        read_lock(name) do
          begin
            ...
            raise SiloError, "Can't determine type for #{describe name}: #{e.message}"
          end
        end
      end
      ...
      def md5(name)
        read_lock(name) do
          begin
            ...
            raise SiloError, "Can't determine md5 for #{describe name}: #{e.message}"
          end
        end
      end
      ...
      def sha1(name)
        read_lock(name) do
          begin
            ...
            raise SiloError, "Can't determine sha1 for #{describe name}: #{e.message}"
          end
        end
      end
      ...
      def datetime(name)
        read_lock(name) do
          begin
            ...
            raise SiloError, "Can't determine the date time for #{describe name}: #{ex.message}"
          end
        end
      end
